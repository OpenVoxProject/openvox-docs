...
In this example, the current version of `stdlib` is 4.8.0, and version 5.0.0 is not yet released.
Under the rules of semantic versioning, 5.0.0 is likely to have incompatibilities, but every version of 4.x should be compatible.
We don't know yet if the module will be compatible with 5.x.
So we set the upper bound of the version dependency to less than the next known incompatible release (or major version).
...
--- | --- | ---
name | Required | The full name of your module, including the Puppet Forge username ("username-module").
version | Required | The current version of your module. This should follow semantic versioning.
author | Required | The person who gets credit for creating the module. If absent, this key defaults to the username portion of the name key.
...
dependencies | Required | An array of other modules that your module depends on to function. See the related topic about specifying dependencies for more details.
requirements | Optional | A list of external requirements for your module, given in the form:<br>`"requirements": [ {"name": "puppet", "version_requirement": "4.x"}].`<br>For details, see the related topic about specifying Puppet version requirements.
project_page | Optional | A link to your module's website to be included on the Forge.
issues_url | Optional | A link to your module's issue tracker.
operatingsystem_support | Optional | An array of operating systems your module is compatible with. See the related topic about specifying operating system compatibility for more details.
tags | Recommended: four to six tags | An array of key words to help people find your module (not case sensitive). For example: ["msyql", "database", "monitoring"]<br>Tags cannot contain whitespace. Certain tags are prohibited, including profanity and anything resembling the `$::operatingsystem` fact (such as redhat, rhel, debian, solaris, aix, windows, or osx). Use of prohibited tags lowers your module's quality score on the Forge.
