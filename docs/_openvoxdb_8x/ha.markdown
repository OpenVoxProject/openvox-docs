...
# Configuring OpenVoxDB for High Availability

In Puppet Enterprise 2016.5 and later, OpenVoxDB may be configured for high
availability in order to withstand network partitions or system failure.

OpenVoxDB is automatically configured for high availability as part of an HA
...
OpenVoxDB HA has two parts: first, Puppet Server is configured to send commands
and queries to multiple OpenVoxDB servers. Second, those servers are configured
...
OpenVoxDB replication is in principle a multi-leader system. You can issue
...
OpenVoxDB HA is available in Puppet Enterprise 2016.5 and later.

Manual Installation and Configuration
----

1. Provision two OpenVoxDB nodes. Designate one of these as your _primary_
   OpenVoxDB, and the other as the _replica_. Be sure that each OpenVoxDB is using
   ...
   `primary-openvoxdb.mycorp.net` and `replica-openvoxdb.mycorp.net`.

2. Configure each OpenVoxDB to pull from the other. This guide will use
  ...
  a. Configure primary-puppetdb to pull from replica-puppetdb by placing this in its
     config file:
         ...
         [sync]
         server_urls = https://replica-openvoxdb.mycorp.net:8081
         intervals = 2m

  b. Configure replica-puppetdb to pull from primary-puppetdb:

         [sync]
         server_urls = https://primary-openvoxdb.mycorp.net:8081
         intervals = 2m

3. Configure the Puppet Server to point to both of your OpenVoxDB instances. In
   your puppetdb.conf, use this as your server configuration (in the 'main' section):

        server_urls = https://primary-openvoxdb.mycorp.net:8081,https://replica-openvoxdb.mycorp.net:8081
        sticky_read_failover = true
        command_broadcast = true

   It is important that your primary OpenVoxDB appear first in this list, so it
   will always be tried first when submitting new data from the Puppet Server.

4. Restart your OpenVoxDBs and your Puppet Server.

Deployment tips
----

- Each OpenVoxDB server should be configured to use a separate PostgreSQL
  instance.

- If you are using multiple compile Server, be sure that their clocks are in
  sync (using NTP, for example). OpenVoxDB relies on timestamps for discerning
...
- The timeout for HTTP connections made by Puppet Server, including those to
  OpenVoxDB, can be configured using the
  ...
  OpenVoxDB HA to work well, you should use a considerably smaller timeout. The
...
A: The principal goal of OpenVoxDB's HA solution is to ensure that Puppet runs
...
by the application. For OpenVoxDB, handling such conflicts at the database level
...
them. In particular: because OpenVoxDB is *eventually* consistent, changes to
exported resources will *eventually* be visible on other nodes (not
immediately). This is true in any OpenVoxDB installation, but there is an added
...
can happen if commands have been written to your primary OpenVoxDB but haven't yet
been copied to the replica. If the primary OpenVoxDB becomes unreachable
...
puppetdb.conf, as recommended above. By pushing all data to the replica OpenVoxDB
...
recommend using OpenVoxDB HA in conjunction with exported resources for sensitive
...
Q: *What durability guarantees does OpenVoxDB HA offer?*
...
stored on at least 2 (or more) OpenVoxDB servers, you can put this in your
puppetdb.conf:

        min_successful_submissions = 2

With this configuration, a Puppet run will fail if any data cannot be written to
at least two OpenVoxDB servers. This may be useful if you have three OpenVoxDB
servers, for example.

## Operations

OpenVoxDB provides several facilities to help you monitor the state of
replication:
...
* query: The query issued to the remote OpenVoxDB server to retrieve this record
...
For basic information on metrics in OpenVoxDB, see
...
* `last-successful-sync-time` (timestamp): The wall-clock time, on the OpenVoxDB
  server, of when the last successful sync finished.
* `last-failed-sync-time` (timestamp): The wall-clock time, on the OpenVoxDB
