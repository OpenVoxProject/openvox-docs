...
title: "API overview"
layout: default
canonical: "/openvoxdb/latest/api/overview.html"
---
...
Because OpenVoxDB collects lots of data from Puppet, it's an ideal platform for new tools and applications that use that data. You can use the HTTP API described in these pages to interact with OpenVoxDB's data.
...
Unlike the query API, these commands are generally only useful to Puppet itself, and all format conversion and command submission is handled by the [OpenVoxDB-termini][termini] on your Puppet Server.
