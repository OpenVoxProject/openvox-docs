...
12. From the command line on your OpenVox Server, run `puppet parser validate site.pp` to ensure that there are no errors. The parser will return nothing if there are no errors.

13. From the command line on your OpenVox agent, use `puppet agent -t` to trigger a Puppet run.
