...
> an accepted fallback format for binary content that cannot be represented as valid UTF-8.

PSON is a variant of [JSON](http://json.org) that OpenVox uses for serializing data to transmit across the network
or store on disk. Whereas JSON requires that the serialized form is valid unicode (usually UTF-8), PSON is 8-bit
ASCII, which allows it to represent arbitrary byte sequences in strings.
...
In all these examples there is a file available called `data.pson` that contains the ruby structure
