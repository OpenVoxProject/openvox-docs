...
write a plugin, get familiar with [writing tasks](writing_tasks.html).
...
- [Writing tasks: Using structured input and
  output](writing_tasks.html#using-structured-input-and-output)
...
can [set Bolt's log level](logs.html#setting-log-level).
...
- [Writing tasks: Returning errors in
  tasks](writing_tasks.html#returning-errors-in-tasks)
