...
- `targets` (array of objects):
  ...
  - `config` (object):
    ...
    configuration reference](bolt_transports_reference.html) for more information
  ...
  - `plugin_hooks` (object):
    The target's [plugin hooks](writing_plugins.html#plugin-hooks).
...
- `metadata` (object):
  The [task's metadata](writing_tasks.html#task-metadata-fields).
