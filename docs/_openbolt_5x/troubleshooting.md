...
You can resolve this by [configuring an alternate tmpdir](bolt_transports_reference.html) for the
...
`host-key-check: false` option under the `config: ssh` section of [inventory.yaml](inventory_files.html).
...
- When you want to view all logs at a specific level on the console, [set the
  log level](logs.html#setting-log-level) from the command line or in your
  configuration file.
