...
> plans](writing_yaml_plans.html).

📖 **Related information**

- For information on using Hiera data in plans, see [Using Bolt with Hiera](hiera.html).
...
- [Task metadata types](writing_tasks.html#common-task-data-types)
...
Each [execution function](plan_functions.html#) returns an object type
`ResultSet`. For each target that the execution takes place on, this object
contains a `Result` object. The [apply action](applying_manifest_blocks.html#)
...
types](bolt_types_reference.html).
...
- [Adding parameters to metadata](writing_tasks.html#)
...
target
definitions](inventory_files.html#target-object).
...
- [Connecting Bolt to PuppetDB](bolt_connect_puppetdb.html)
