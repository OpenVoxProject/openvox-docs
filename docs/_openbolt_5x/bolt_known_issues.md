...
such as [`bolt-defaults.yaml`](bolt_defaults_reference.html) or
[`bolt-project.yaml`](bolt_project_reference.html), in an
[inventory file](bolt_inventory_reference.html), or passing the password on the
...
or set `concurrency: <CONCURRENCY>` in [Bolt config](configuring_bolt.html).
