...
blocks](applying_manifest_blocks.html#applying-manifest-blocks-from-a-puppet-plan).
...
page](bolt_transports_reference.html). The Podman transport doesn't support the `service-url`
...
see the full list of supported configuration options [on the transport reference
page](bolt_transports_reference.html). The jail transport doesn't support the `service-url`
configuration options as the transport doesn't support remote connections. If this is a feature
you're interested in, let us know [in Slack](https://slack.puppet.com) or submit a [Github
issue](https://github.com/puppetlabs/bolt/issues).
...
about this function in [Bolt plan functions](plan_functions.html#set_resources).
...
using [the data type's functions](bolt_types_reference.html#resourceinstance).
...
types](bolt_types_reference.html#resourceinstance).
...
See [Transport configuration options](bolt_transports_reference.html#ssh) for a list
