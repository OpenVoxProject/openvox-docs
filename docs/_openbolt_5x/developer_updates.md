...
- **New default modulepath**
  ...
  modulepath, see the [modules overview](modules.html).
...
- **Ship with `bolt.bat` file on Windows**
  ...
  cmdlets](bolt_cmdlet_reference.html).
...
- **`apply_settings` configuration option renamed to `apply-settings`**
  ...
  options](bolt_project_reference.html) reference page.
...
- **`source` key for YAML plan `upload` step renamed to `upload`**
  ...
  plans](writing_yaml_plans.html#file-upload-step).
...
files, see [Migrate a Bolt project](projects.html#migrate-a-bolt-project).
...
try it out, [create a
project](bolt_installing_modules.html#create-a-bolt-project-with-pre-installed-modules)
or [migrate an existing project](./projects.html#migrate-a-bolt-project).
