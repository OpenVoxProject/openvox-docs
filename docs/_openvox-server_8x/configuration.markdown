---
layout: default
title: "OpenVox Server Configuration"
---
...
At startup, OpenVox Server reads all the `.conf` files in the `conf.d` directory. You must restart OpenVox Server for any changes to those files to take effect. The `conf.d` directory contains the following files
and settings:
...
OpenVox Server's logging is routed through the JVM [Logback](http://logback.qos.ch/) library. The default Logback configuration file is at `/etc/puppetserver/logback.xml` or
...
OpenVox Server logs HTTP traffic in a format similar to Apache, and to a separate file than the main log file. By default, this is located at `/var/log/puppetlabs/puppetserver/puppetserver-access.log`.
...
To enable additional logging related to `auth.conf`, edit OpenVox Server's `logback.xml` file. By default, only a single message is logged when a request is denied.

To enable a one-time logging of the parsed and transformed `auth.conf` file, add the following to OpenVox Server's `logback.xml` file:

```text
<logger name="puppetlabs.trapperkeeper.services.authorization.authorization-service" level="DEBUG"/>
```

To enable rule-by-rule logging for each request as it's checked for authorization, add the following to OpenVox Server's `logback.xml` file:
...
OpenVox Server is built on top of our open-source Clojure application framework, [Trapperkeeper](https://github.com/openvoxproject/trapperkeeper).

One of the features that Trapperkeeper provides is the ability to enable or disable individual services that an application provides. In OpenVox Server, you can use this feature to enable or disable the CA
...
You can provide Java JARs to be loaded upon OpenVox Server's startup.

When launched, OpenVox Server automatically loads any JARs placed in `/opt/puppetlabs/server/data/puppetserver/jars` into the `classpath`.
JARs placed here will not be modified or removed when upgrading OpenVox Server.
